...
Go to the _Project information_ tab, and check the following:
...
- [ ] Go to *_Quality Assurance system* > Projects > Ongoing projects > `<project_name>`
...
- [ ] _All samples sequenced_ field is signed
- [ ] If required, _Best Practice analysis_ field is signed
- [ ] _All raw data delivered_ field is signed
- [ ] All samples show the correct number of reads
  - If not, click on _Reads aggregation | Run_ and then _Record changes | Run_ and double check the report send to the user
...
Go to the _Project and Samples_ tab and search for <project_name>.
...
```text
Project closed in LIMS. <author_signature>
```
